...
app.post('/api/auth/sign-in', (req, res, next) => {
  ...
  db.query(sql, params)
    .then(results => {
      const returnRes = results.rows[0];
      if (!returnRes) {
        res.status(401).json({ message: 'invalid login' });
      } else {
        argon2.verify(returnRes.hashedPassword, password)
          .then(result => {
            if (!result) {
              res.status(401).json({ message: 'invalid login' });
            } else {
              const payload = { userInfo: { userId: returnRes.userId, hashedPassword: returnRes.hashedPassword } };
              const token = jwt.sign(payload, process.env.TOKEN_SECRET);
              res.status(200).json({ message: { userId: payload.userInfo.userId, token: token } });
            }
          })
          .catch(err => {
            next(err);
